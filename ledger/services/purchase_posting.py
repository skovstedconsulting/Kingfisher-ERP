from django.db import transaction
from decimal import Decimal
from django.utils import timezone

from ledger.models import Journal, JournalLine, OpenItem
from ledger.services.fx import get_fx_rate
from inventory.models import InventoryLayer, StockMove
from core.services.vat import vat_code_allowed_for_debtor_area  # not used here but kept for symmetry

@transaction.atomic
def post_purchase_invoice(doc, by_user=None):
    """Post a purchase invoice.

    What it does (high level):
    1) Lock the document row to prevent double posting
    2) Create a draft Journal
    3) For each purchase line:
       - Determine expense vs inventory posting
       - Determine VAT (input VAT)
       - Add JournalLines in base and tx currency
       - If stock item: create FIFO inventory layer
    4) Add AP control line (credit)
    5) Post the journal (FSM transition draft->posted)
    6) Create an OpenItem (AP)
    7) Stamp document posted fields + totals

    Notes:
    - This is a readable baseline. Later you can add:
      * freight lines
      * landed cost allocation
      * partial receipts
      * invoice matching
      * FX revaluation for open items
    """

    # Re-load and lock the doc row
    doc = type(doc).objects.select_for_update().select_related("entity", "currency", "creditor").get(pk=doc.pk)

    if doc.posted_journal_id:
        raise ValueError("Document is already posted.")
    if not doc.lines.exists():
        raise ValueError("Document has no lines.")

    entity = doc.entity
    base_ccy = entity.base_currency

    # FX logic (simple baseline)
    fx_rate = Decimal("1.0")
    if doc.currency_id != base_ccy.pk:
        fx_rate = get_fx_rate(entity, doc.date, base_ccy.code, doc.currency.code)

    # Create draft journal
    j = Journal.objects.create(
        entity=entity,
        date=doc.date,
        reference=f"Purchase invoice {doc.invoice_no or doc.pk}",
    )

    def add_line(account, *, debit_base=Decimal("0.00"), credit_base=Decimal("0.00"),
                 debit_tx=Decimal("0.00"), credit_tx=Decimal("0.00"),
                 currency=None, fx_rate=None, description=""):
        """Create a JournalLine (helper for readability)."""
        JournalLine.objects.create(
            journal=j,
            account=account,
            description=description[:255],
            currency=currency,
            fx_rate=fx_rate,
            debit_base=debit_base,
            credit_base=credit_base,
            debit_tx=debit_tx,
            credit_tx=credit_tx,
        )

    total_net_tx = Decimal("0.00")
    total_net_base = Decimal("0.00")
    total_vat_tx = Decimal("0.00")
    total_vat_base = Decimal("0.00")

    for line in doc.lines.select_related("item", "vat_code", "item__group").all():
        # Net amounts
        net_tx = line.net_tx
        net_base = (net_tx / fx_rate).quantize(Decimal("0.01")) if fx_rate != 0 else Decimal("0.00")

        # VAT code: line overrides, else from item group
        vat_code = line.vat_code
        if not vat_code and line.item_id and line.item.group_id:
            vat_code = line.item.group.default_purchase_vat_code

        vat_tx = Decimal("0.00")
        vat_base = Decimal("0.00")
        if vat_code and vat_code.rate:
            # Purchase VAT is input VAT; deduction_rate may apply
            deduction = vat_code.deduction_rate if vat_code.deduction_rate is not None else Decimal("1.0")
            vat_tx = (net_tx * vat_code.rate * deduction).quantize(Decimal("0.01"))
            vat_base = (vat_tx / fx_rate).quantize(Decimal("0.01")) if fx_rate != 0 else Decimal("0.00")

        total_net_tx += net_tx
        total_net_base += net_base
        total_vat_tx += vat_tx
        total_vat_base += vat_base

        # Expense vs Inventory decision:
        # - If stock item => debit inventory
        # - Else => debit expense
        if not line.item_id or not line.item.group_id:
            raise ValueError("Purchase line must have an item with a group for posting.")

        group = line.item.group

        if line.item.is_stock_item:
            if not group.inventory_account_id:
                raise ValueError(f"ItemGroup {group} missing inventory_account.")
            add_line(
                account=group.inventory_account,
                debit_base=net_base,
                debit_tx=net_tx,
                currency=doc.currency,
                fx_rate=fx_rate,
                description=line.description or "Inventory purchase",
            )

            # Create FIFO layer (in base currency)
            InventoryLayer.objects.create(
                entity=entity,
                item=line.item,
                qty_remaining=line.qty,
                unit_cost_base=(net_base / line.qty).quantize(Decimal("0.0001")) if line.qty else Decimal("0.0000"),
            )

            StockMove.objects.create(
                entity=entity,
                item=line.item,
                date=doc.date,
                qty=line.qty,
                unit_cost_base=(net_base / line.qty).quantize(Decimal("0.0001")) if line.qty else Decimal("0.0000"),
                purchase_line=line,
            )
        else:
            if not group.expense_account_id:
                raise ValueError(f"ItemGroup {group} missing expense_account.")
            add_line(
                account=group.expense_account,
                debit_base=net_base,
                debit_tx=net_tx,
                currency=doc.currency,
                fx_rate=fx_rate,
                description=line.description or "Expense",
            )

        # Input VAT line
        if vat_tx != 0:
            if not vat_code.input_vat_account_id:
                raise ValueError(f"VAT code {vat_code.code} missing input_vat_account.")
            add_line(
                account=vat_code.input_vat_account,
                debit_base=vat_base,
                debit_tx=vat_tx,
                currency=doc.currency,
                fx_rate=fx_rate,
                description=f"Input VAT {vat_code.code}",
            )

    gross_tx = (total_net_tx + total_vat_tx).quantize(Decimal("0.01"))
    gross_base = (total_net_base + total_vat_base).quantize(Decimal("0.01"))

    # AP control line (credit)
    ap = entity.default_ap_account
    if not ap:
        raise ValueError("Entity.default_ap_account is required for purchase posting.")
    add_line(
        account=ap,
        credit_base=gross_base,
        credit_tx=gross_tx,
        currency=doc.currency,
        fx_rate=fx_rate,
        description="Accounts payable",
    )

    # Post the journal (FSM transition)
    j.post(by_user=by_user)

    # Create AP open item for settlement
    OpenItem.objects.create(
        entity=entity,
        kind=OpenItem.Kind.AP,
        creditor=doc.creditor,
        purchase_document=doc,
        currency=doc.currency,
        original_tx=gross_tx,
        original_base=gross_base,
        remaining_tx=gross_tx,
        remaining_base=gross_base,
        due_date=doc.date,  # later: use creditor group delivery/payment terms
    )

    # Stamp document
    doc.posted_journal = j
    doc.posted_at = timezone.now()
    doc.posted_by = by_user
    doc.total_tx = gross_tx
    doc.total_base = gross_base
    doc.save(update_fields=["posted_journal", "posted_at", "posted_by", "total_tx", "total_base"])
